...
import { credentialsAreManaged, getCredentials, getSetting } from '@/lib/db';
...
export default async function SettingsPage() {
  const managed = credentialsAreManaged();
  ...
  return (
    <div className="max-w-2xl space-y-8 pb-12">
      ...
      <div className="bg-slate-900 dark:bg-slate-800 rounded-[2rem] p-10 text-white shadow-2xl shadow-slate-200 dark:shadow-none relative overflow-hidden border border-slate-800 dark:border-slate-700">
        <div className="relative z-10">
          ...
          <div className="mt-4 flex items-center gap-4">
            <span className="text-6xl font-mono font-bold tracking-tighter leading-none">{connected ? `$${balance.toFixed(2)}` : 'Unavailable'}</span>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm p-1">
        <div className="bg-slate-50/80 dark:bg-slate-800/40 rounded-[1.4rem] p-8">
          <form action={updateSettings} className="space-y-8">
            {/* API Credentials */}
            {managed && <p className="text-sm text-slate-500">DataForSEO is configured by your administrator. Search defaults can be changed below.</p>}
            <fieldset disabled={managed}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input name="login" type="text" defaultValue={managed ? '' : creds?.login || ''} className={inputCls} />
                </div>
                <div className="space-y-2">
      ...
      {/* Danger Zone */}
      {!managed && <div className="bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-900 rounded-3xl p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
